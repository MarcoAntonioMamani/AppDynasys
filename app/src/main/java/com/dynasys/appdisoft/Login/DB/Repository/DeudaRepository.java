...
public class DeudaRepository {
    ...
    public DeudaEntity getDeuda(int noteId) throws ExecutionException, InterruptedException {
        return new DeudaRepository.getDeudaAsync(mDeudadDao).execute(noteId).get();
    }
    public List<DeudaEntity> getMDeudaAll(int clienteId) throws ExecutionException, InterruptedException {
        return new DeudaRepository.getMDeudaAllAsync(mDeudadDao).execute(clienteId).get();
    }
    public void insertDeuda(DeudaEntity user) {
        new DeudaRepository.insertDeudaAsync(mDeudadDao).execute(user);
    }

    public void updateDeuda(DeudaEntity user) {
        new DeudaRepository.updateDeudaAsync(mDeudadDao).execute(user);
    }

    public void deleteDeuda(DeudaEntity user) {
        new DeudaRepository.deleteDeudaAsync(mDeudadDao).execute(user);
    }

    public void deleteAllDeuda() {
        new DeudaRepository.deleteAllDeudaAsync(mDeudadDao).execute();
    }
