package com.dynasys.appdisoft.RevisarProductos.FisicoProductos;
...
import com.dynasys.appdisoft.Adapter.AdapterDetalleFisico;
 ...
 */
public class FisicoProductosFragment extends Fragment implements SearchView.OnQueryTextListener {
    private List<AlmacenEntity> lisAlmacen=new ArrayList<>();
    View view;
    Context context;
    RecyclerView recList;
    public AdapterDetalleFisico adapterPerfil;
    TextView tvTotalSaldo,tvTotalFisico,tvTotalDiferencia;
    ...
    Button btnSalir;
    public FisicoProductosFragment() {
        // Required empty public constructor
    }
    @Override
    public void onResume() {
        super.onResume();
        getActivity().setTitle("Fisico Producto");
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,Bundle savedInstanceState) {
        ...
        view= inflater.inflate(R.layout.fragment_fisico_productos, container, false);

        recList = (RecyclerView) view.findViewById(R.id.id_detalle_listreview_fisico);
        recList.setHasFixedSize(true);
        simpleSearchView = (SearchView) view.findViewById (R.id.simpleSearchViewFisicoProduct);
        simpleSearchView.setOnQueryTextListener((SearchView.OnQueryTextListener) this);
        simpleSearchView.setIconifiedByDefault(false);
        btnSalir=view.findViewById(R.id.id_btn_fisico_cancelar);
        tvTotalSaldo=view.findViewById(R.id.view_info_fisico_saldo);
        tvTotalFisico=view.findViewById(R.id.view_info_fisico_fisico);
        tvTotalDiferencia =view.findViewById(R.id.view_info_fisico_diferencia);
        tvTotalSaldo.setText(ShareMethods.ObtenerDecimalToString(0,2));
        tvTotalFisico.setText(ShareMethods.ObtenerDecimalToString(0,2));
        tvTotalDiferencia.setText(ShareMethods.ObtenerDecimalToString(0,2));
    ...
    public void iniciarParametroFecha(){
        tvDesde=(TextView)view.findViewById(R.id.id_tvfecha_fisico_desde);
        tvHasta=(TextView)view.findViewById(R.id.id_tvfecha_fisico_hasta);
        tvDesde.setEnabled(false);
        tvHasta.setEnabled(false);
        btnDesde=(ImageButton) view.findViewById(R.id.ib_btn_fisico_desde);
        btnHasta=(ImageButton)view.findViewById(R.id.id_btn_fisico_hasta);
        btnCargar=(Button)view.findViewById(R.id.id_btn_fisico_buscar) ;
    ...
    public void Filtrar() throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < lisAlmacen.size(); i++) {
            ...
            Fisico+=lisAlmacen.get(i).getFisico();
            Diferencia+=lisAlmacen.get(i).getDiferencia();
        }

        tvTotalSaldo.setText(ShareMethods.ObtenerDecimalToString(Saldo,2));
        tvTotalFisico.setText(ShareMethods.ObtenerDecimalToString(Fisico,2));
        tvTotalDiferencia.setText(ShareMethods.ObtenerDecimalToString(Diferencia,2));
    ...
    public void CargarRecycler(List<AlmacenEntity> listEfectivo){
        if (listEfectivo!=null){
            adapterPerfil = new AdapterDetalleFisico(context,lisAlmacen);
